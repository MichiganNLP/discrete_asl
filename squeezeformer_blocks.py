...
class ConvModule(nn.Module):
    ...
    def forward(self, x, mask_pad):
        ...
        x = x.masked_fill(~mask_pad, 0.0) 
        # THIS LINE ABOVE FUCKING FIXES the padding bug.
        # THE PROBLEM IS THAT WHEN I ADD PADDING THE DEPTHWISE CONVOLUTIONS INCLUDES VALUES FROM THE PADDING
        # AND IT CHANGES THE VALUES FOR NON-PADDING FRAMES
        x = self.dw_conv(x)
...
class SqueezeformerBlock(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        bs, slen, nfeats = x.shape # those values are correct
        residual = x
        ...
        x = residual + self.conv(x, mask_pad=mask.bool().unsqueeze(1)) # TODO: THIS IS BUGGY
        # Skip pad #2
        x_skip = x.view(-1, x.shape[-1])
        x = x_skip[mask_flat].unsqueeze(0)
...
class ConvModuleWithDownsampling(nn.Module):
    """
    Convolution module starts with a pointwise convolution and a gated linear unit (GLU).
    This is followed by a single 1-D depthwise convolution layer. Batchnorm is deployed just after the convolution
    to aid training deep models.
    ...
    def __init__(
        ...
        super(ConvModuleWithDownsampling, self).__init__()
        assert (kernel_size - 1) % 2 == 0, "kernel_size should be a odd number for 'SAME' padding"
        assert expansion_factor == 2, "Currently, Only Supports expansion_factor 2"

        self.pw_conv_1 = PointwiseConv1d(in_channels, in_channels * expansion_factor, stride=2, padding=0, bias=True)
        self.act1 = GLU(dim=1)
        self.dw_conv = DepthwiseConv1d(in_channels, in_channels, kernel_size, stride=1, padding=(kernel_size - 1) // 2)
        self.bn = nn.BatchNorm1d(in_channels)
        self.act2 = Swish()
        self.pw_conv_2 = PointwiseConv1d(in_channels, in_channels, stride=2, padding=0, bias=True)
        self.do = nn.Dropout(p=dropout_p)
    ...
    def forward(self, x, mask_pad):
        ...
        x = self.dw_conv(x)
        x_bn = x.permute(0, 2, 1).reshape(-1, x.shape[1])
        mask_pad = interpolate(mask_pad.float(), scale_factor=0.5).bool()
        mask_bn = mask_pad.view(-1)
...
class PointwiseConv1dTranspose(nn.Module):
    """
    When kernel size == 1 conv1d, this operation is termed in literature as pointwise convolution.
    This operation often used to match dimensions.
    ...
    def __init__(
        ...
        super(PointwiseConv1dTranspose, self).__init__()
...
class SqueezeformerBlockWithDownsampling(nn.Module):
    """
    SqueezeformerBlock is a simpler block structure similar to the standard Transformer block,
    where the MHA and convolution modules are each directly followed by a single feed forward module.
    ...
    def __init__(
        ...
        super(SqueezeformerBlockWithDownsampling, self).__init__()
        ...
        self.ln_ff_mhsa = nn.LayerNorm(encoder_dim)
        self.conv = ConvModuleWithDownsampling(
            in_channels=encoder_dim,
            kernel_size=conv_kernel_size,
            expansion_factor=conv_expansion_factor,
            dropout_p=conv_dropout_p,
    ...
    def forward(self, x, mask):
        ...
        # NO RESIDUALS HERE - BUT ONLY FOR ONE BLOCK SO HOPEFULLY OK
        x = self.conv(x, mask_pad=mask.bool().unsqueeze(1))

        # Skip pad #2
        x_skip = x.reshape(-1, x.shape[-1])
        mask_flat = interpolate(mask.unsqueeze(1).float(), scale_factor=0.25).view(-1).bool()
        x = x_skip[mask_flat].unsqueeze(0)
        ...
        # Unskip pad #2
        x_skip[mask_flat] = x[0]
        x = x_skip.view(bs, int(slen/4), nfeats)

        return x

class ConvModuleWithUpsampling(nn.Module):
    """
    Convolution module starts with a pointwise convolution and a gated linear unit (GLU).
    This is followed by a single 1-D depthwise convolution layer. Batchnorm is deployed just after the convolution
    to aid training deep models.
    ...
    def __init__(
        ...
        super(ConvModuleWithUpsampling, self).__init__()
        assert (kernel_size - 1) % 2 == 0, "kernel_size should be a odd number for 'SAME' padding"
        assert expansion_factor == 2, "Currently, Only Supports expansion_factor 2"

        self.pw_conv_1 = PointwiseConv1dTranspose(in_channels, in_channels * expansion_factor, stride=2, padding=0, bias=True, output_padding=1)
        self.act1 = GLU(dim=1)
        self.dw_conv = DepthwiseConv1d(in_channels, in_channels, kernel_size, stride=1, padding=(kernel_size - 1) // 2)
        self.bn = nn.BatchNorm1d(in_channels)
        self.act2 = Swish()
        self.pw_conv_2 = PointwiseConv1dTranspose(in_channels, in_channels, stride=2, padding=0, bias=True, output_padding=1)
        self.do = nn.Dropout(p=dropout_p)
    ...
    def forward(self, x, mask_pad):
        ...
        Reference for masking : https://github.com/Ascend/ModelZoo-PyTorch/blob/master/PyTorch/built-in/audio/Wenet_Conformer_for_Pytorch/wenet/transformer/convolution.py#L26
        """
        mask_pad_original = mask_pad
        mask_pad = interpolate(mask_pad_original.float(), scale_factor=0.25).bool()
        # mask batch padding
        x = x.transpose(1, 2)
        if mask_pad.size(2) > 0:  # time > 0
            x = x.masked_fill(~mask_pad, 0.0)
        ...
        x = self.dw_conv(x)
        x_bn = x.permute(0, 2, 1).reshape(-1, x.shape[1])
        mask_pad = interpolate(mask_pad_original.float(), scale_factor=0.5).bool()
        mask_bn = mask_pad.view(-1)
...
class SqueezeformerBlockWithUpsampling(nn.Module):
    """
    SqueezeformerBlock is a simpler block structure similar to the standard Transformer block,
    where the MHA and convolution modules are each directly followed by a single feed forward module.
    ...
    def __init__(
        ...
        super(SqueezeformerBlockWithUpsampling, self).__init__()
        ...
        self.ln_ff_mhsa = nn.LayerNorm(encoder_dim)
        self.conv = ConvModuleWithUpsampling(
            in_channels=encoder_dim,
            kernel_size=conv_kernel_size,
            expansion_factor=conv_expansion_factor,
            dropout_p=conv_dropout_p,
    ...
    def forward(self, x, mask):
        original_mask = mask
        mask_pad = (mask).long().bool().unsqueeze(1)
        mask_pad = interpolate(mask_pad.float(), scale_factor=0.25).bool()
        mask_pad = ~(mask_pad.permute(0, 2, 1) * mask_pad)
