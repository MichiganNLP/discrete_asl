    ...
    def forward(self, x, mask):
        ...
        if self.training:
            if self.commitment_weight > 0:
                ...
                commit_loss = (commit_loss * mask.unsqueeze(2)).sum()

                non_zero_elements = mask.sum()
                commit_loss = commit_loss / non_zero_elements
                loss = loss + commit_loss * self.commitment_weight

                # embed_loss = F.mse_loss(quantize, x.detach(), reduction='none')
                # embed_loss = (embed_loss * mask.unsqueeze(2)).sum()
                # non_zero_elements = mask.sum()
                # embed_loss = embed_loss / non_zero_elements
                # loss = loss + embed_loss * self.commitment_weight
...
class ResidualVectorQuantization(nn.Module):
    ...
    def forward(self, x, mask, n_q: tp.Optional[int] = None):
        quantized_out = 0.0
        residual = x
        ...
        for i, layer in enumerate(self.layers[:n_q]):
            quantized, indices, loss = layer(residual, mask)
...
class ResidualVectorQuantizer(nn.Module):
    ...
    def __init__(
        ...
        self.vq = ResidualVectorQuantization(
            ...
            channels_last=True #CHANGED: TODO CHANGE BACK
        )

    def forward(self, x: torch.Tensor, mask, frame_rate: int):
        n_q = self.n_q
        if self.training and self.q_dropout:
            n_q = int(torch.randint(1, self.n_q + 1, (1,)).item())
        bw_per_q = math.log2(self.bins) * frame_rate / 1000
        quantized, codes, commit_loss = self.vq(x, mask=mask, n_q=n_q)
